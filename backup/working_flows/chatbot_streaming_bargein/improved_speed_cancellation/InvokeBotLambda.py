...
def _enhanced_chat_and_stream(event):
    ...
    try:
        ...
        deadline = time.time() + float(os.getenv("FIRST_PART_WAIT_SEC", "2.5"))
...
def _cancel_previous_job_if_any(event):
    # Try contact attributes first
    attrs = (event.get("Details", {}) or {}).get("ContactData", {}).get("Attributes", {}) or {}
    prev = attrs.get("JobId")
    if not prev:
        return
    try:
        cancel_url = f"{TTS_URL.rstrip('/').replace('synthesize', 'synthesize_stream_cancel')}"
        _http_post_json(cancel_url, {"job_id": prev}, token=TTS_TOKEN, timeout=3)
        _log("Cancelled previous TTS job", prev_job_id=prev[:8] + "…")
    except Exception as e:
        _log("Cancel previous TTS job failed (continuing)", error=str(e))
...
def _enhanced_chat_and_stream_cached(event):
    """
    Unified cache + stream handler.

    Behavior:
    - Cancel previous stream job (if any).
    - Compute pk from user speech (+chat flag). Check DDB:
        * ready  -> return immediate single-clip playback (and Prompt ARN)
        * pending-> start streaming now (do NOT recreate worker)
        * miss   -> create pending, invoke worker to build cached final clip, start streaming now
    - Always include `cache_hit`, `pk`, `job_bucket`, `job_key` in attributes.
    """

    # 0) Cancel any prior streaming job to avoid overlap
    _cancel_previous_job_if_any(event)

    # 1) Extract input + chat flag
    user_speech = _extract_user_speech_input(event) or "안녕하세요"
    ...
    if chat_on is None:
        ...
        chat_on = str(chat_on).lower() in ("1", "true", "y", "yes")

    # pk based on the *input* + chat flag (matches existing make_pk semantics)
    pk = make_pk(user_speech, sr=8000, lang="ko-KR", voice="Jihye", chat=chat_on)
    item = ddb_get(pk) or {}

    session_id = _session_id_from_event(event, uuid.uuid4().hex)

    # 2) FAST PATH: ready cache → return immediate single-clip playback (no new stream)
    if item.get("state") == "ready" and item.get("key"):
        bucket = item.get("bucket", COMPANY_BUCKET)
        key    = item["key"]
        url    = to_regional_url(key)

        # Ensure we have a Prompt ARN (use stored or create/update)
        prompt_arn = item.get("prompt_arn", "")
        if not prompt_arn:
            try:
                _, prompt_arn = _ensure_connect_prompt_for_key(bucket, key, pk[:12])
            except Exception as e:
                _log("Prompt ensure failed (continuing without ARN)", error=str(e))
                prompt_arn = ""
        ...
        attrs = {
            ...
            "ready": "true",
            "cache_hit": "true",
            "JobId": "",  # no active streaming job on cache-hit
            "UserInput": user_speech,
            "ChatAnswer": item.get("final_text", ""),
            "job_bucket": bucket,
            "job_key": key,
            "pk": pk,
        }
        if prompt_arn:
            attrs["PromptARN0"] = prompt_arn
    ...
    # 3) If pending or miss, we will stream now. Also ensure the cache pipeline exists.

    # If miss → create pending + fire worker to build the single-file cached WAV
    created_pending = False
    cache_bucket = item.get("bucket", COMPANY_BUCKET)
    cache_key    = item.get("key")
    if not item:
        # deterministic cache object for this request
        cache_key = _make_key(uuid.uuid4().hex)
        created_pending = ddb_put_pending_if_absent(pk, cache_bucket, cache_key, user_speech)
        _log("Cache miss - pending created", pk=pk[:12]+"...", created=created_pending, key=cache_key)

        # Kick worker (fire-and-forget) to produce final clip & prompt & flip DDB→ready
        if created_pending:
            try:
                lambda_client.invoke(
                    FunctionName=ASYNC_FUNCTION_NAME,
                ...
                _log("Worker invoke failed", error=str(e))

    # 4) Start streaming NOW using the chatbot response for snappy UX
    try:
        chatbot_response = call_chatbot(user_speech, session_id)
    except Exception as e:
        _log("Chatbot error - falling back to echo", error=str(e))
        chatbot_response = user_speech

    batch_attrs = _start_stream(
        chatbot_response,
        filler_text=filler_text,
        include_prompts=False  # use direct URLs for the stream parts
    )

    # 5) Merge cache metadata + helpful fields for the flow
    #    (If item was pending before, keep that; if we just created pending, it's also a cache scenario)
    is_cache = (item.get("state") == "pending") or created_pending
    batch_attrs.update({
        "cache_hit": "true" if is_cache else "false",
        "pk": pk,
    ...
    _log("Unified stream started",
         pk=pk[:12]+"...",
         cache=is_cache,
         stream_job=batch_attrs.get("JobId","")[:8] if batch_attrs.get("JobId") else "")

    # 6) Return attributes for Connect
    return {"setAttributes": {k: (v if isinstance(v, str) else str(v)) for k, v in batch_attrs.items()}}
...
def lambda_handler(event, context):
    ...
    try:
        if action == "start":
            ...
            return _enhanced_chat_and_stream_cached(event) 
        elif action == "check":
            return _check_ready_by_pk(event)

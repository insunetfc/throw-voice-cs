...
def _run_stream_job_chunked(job_id: str, text: str, target_sr: int):
    ...
    for idx, sent in enumerate(sentences):
        if _is_cancelled(job_id):
            # emit tiny sentinel so pollers stop
            ...
            return
        try:
            # synthesize ONE sentence and upload as part{idx}.wav
            _synthesize_chunk_to_key(sent, target_sr, job_id, idx)  # must write to f"{job_id}/part{idx}.wav"

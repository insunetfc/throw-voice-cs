...
TTS_URL    = os.getenv("TTS_URL", "https://wait-dog-sport-viewer.trycloudflare.com/synthesize")
...
def _check_ready_by_pk(event):
    ...
    _log("Checking ready status by pk", pk=pk[:12] + "...")  # Log first 12 chars for privacy
    ...
    if item.get("state") == "ready":
        _log("Item is ready", pk=pk[:12] + "...", has_prompt_arn=bool(item.get("prompt_arn")))
        return {
            "ready": True,
            "state": "ready",
        ...
        _log("Item still pending", pk=pk[:12] + "...")
        return {
            "ready": False,
            "state": "pending",
            "bucket": item.get("bucket", ""),
...
def _start(event):
    ...
    if item:
        if item.get("state") == "ready" and item.get("prompt_arn"):
            return {
                ...
                "key": item["key"],
                "ready": "true",
                # Top-level for $.External.prompt_arn
                "prompt_arn": item["prompt_arn"],
                "setAttributes": {
                    "ready": "true",
                    "prompt_arn": item["prompt_arn"],
        ...
        if item.get("state") == "pending":
            # Another worker is already producing this audio
            return {
                "job_id": "",
                "bucket": item["bucket"],
                "key": item["key"],
                "ready": "false",
                # No ARN yet, keep top-level defined as empty for safety
                "prompt_arn": "",
                "setAttributes": {
                    "ready": "false",
    ...
    return {
        ...
        "setAttributes": {"job_bucket": COMPANY_BUCKET, "job_key": key, "ready": "false", "pk": pk}
    }



def _check(event):
    bucket = (event.get("Details", {}).get("Parameters", {}).get("bucket")
              or event.get("Details", {}).get("Parameters", {}).get("job_bucket")
              or event.get("bucket") or COMPANY_BUCKET)
    key = (event.get("Details", {}).get("Parameters", {}).get("key")
           or event.get("Details", {}).get("Parameters", {}).get("job_key")
           or event.get("key"))
    if not bucket or not key:
        return {"ready": "false", "setAttributes": {"ready": "false", "audio_url": "", "prompt_arn": ""}, "error": "missing bucket/key"}

    try:
        s3.head_object(Bucket=bucket, Key=key)
    except Exception:
        return {"ready": "false", "setAttributes": {"ready": "false", "audio_url": "", "prompt_arn": ""}}

    url = s3.generate_presigned_url("get_object", Params={"Bucket": bucket, "Key": key}, ExpiresIn=900)

    final_text, prompt_arn = "", ""
    try:
        final_text = s3.get_object(Bucket=bucket, Key=key + ".txt")["Body"].read(2048).decode("utf-8","ignore")
    except Exception:
        pass
    try:
        prompt_arn = s3.get_object(Bucket=bucket, Key=key + ".prompt")["Body"].read(1024).decode("utf-8","ignore")
    except Exception:
        pass

    return {
        "ready": "true",
        "audio_url": url,             # still handy for debugging
        "prompt_arn": prompt_arn,     # <-- what the flow will actually use
        "setAttributes": {"ready": "true", "audio_url": url, "prompt_arn": prompt_arn, "final_text": final_text}
    }
...
def lambda_handler(event, context):
    ...
    try:
        ...
        elif action == "check_ready":  # New action
            return _check_ready_by_pk(event)
        elif action == "get_prompt":  # You might also want to keep this existing one

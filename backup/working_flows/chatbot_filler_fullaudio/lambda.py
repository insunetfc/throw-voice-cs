...
FILLER_BUCKET = os.getenv("FILLER_BUCKET", os.getenv("TTS_BUCKET", "seoul-bucket-65432"))  # bucket where filler WAVs live
...
def _tts_full_with_filler(event):
    # 1) Get input text (reuse your extractor)
    ...
    # 2) Compute cache key (same logic as your existing _start)
    pk = make_pk(text=text, sr=sr)

    # 3) Mark pending in DDB (idempotent)
    ddb_put_pending_if_absent(pk, bucket=os.getenv("TTS_BUCKET",""), key="", text=text)

    # 4) Fire-and-forget worker to synthesize full audio (non-streaming)
    payload = {"action": "tts_full_worker", "text": text, "pk": pk, "sample_rate": sr}
    _lambda.invoke(
    ...
    # 5) Choose a filler and return it immediately
    cat = params.get("filler_category")  # e.g., "시간벌기형"
    ...
    attrs = {
        ...
        "AudioS3UrlCount": "1",
        "BatchCount": "1",
        "HasMore": "true",          # tell the flow to poll for the full audio next
        "NextIndexOut": "0",
        "pk": pk,
...
def _tts_full_worker(event):
    try:
        ...
        res = _http_post_json(
            ...
            {"text": text, "sample_rate": sr},
            token=TTS_TOKEN,
            timeout=SYNTH_HTTP_TIMEOUT_SEC,
        )
        bucket = res.get("bucket") or os.getenv("TTS_BUCKET","")
        key    = res.get("key")
        if not (bucket and key):
            _log("tts_full_worker: missing bucket/key", resp=res)
            return {"ok": False}
...
def _check_full(event):
    ...
    if not pk:
        return {"setAttributes": {"ready": "true", "HasMore": "true", "AudioS3UrlCount": "0", "BatchCount": "0"}}

    item = ddb_get(pk) or {}
    if item.get("state") == "ready" and item.get("bucket") and item.get("key"):
    ...
    # Not ready yet
    return {"setAttributes": {"ready": "true", "HasMore": "true", "AudioS3UrlCount": "0", "BatchCount": "0", "pk": pk}}
...
def _synthesize_full_with_filler(event):
    ...
    Behavior:
    1. Extract user input and get chatbot response
    2. Immediately return filler audio for playback
    3. Start background worker to generate full audio
    4. Store metadata so subsequent calls can check for completion
    
    This gives 2-3 seconds of filler time while the full audio is being generated.
    """
    ...
    try:
        ...
        # Generate cache key for the full audio
        pk = make_pk(chatbot_response, sr=8000, lang="ko-KR", voice="Jihye", chat=False)
        
        # Check if we already have this cached
        item = ddb_get(pk)
        if item and item.get("state") == "ready" and item.get("key"):
            # Cache hit - return the cached audio immediately (no filler needed)
            bucket = item.get("bucket", COMPANY_BUCKET)
            key = item["key"]
            s3_url = f"https://{bucket}.s3.{S3_REGION}.amazonaws.com/{key}"
            
            _log("Cache hit for full synthesis - returning immediately", pk=pk[:12] + "...")
            ...
            return {
                ...
                "AudioS3UrlCount": "1",
                "BatchCount": "1",
                "HasMore": "false",  # Complete audio, no more parts
                "NextIndexOut": "1",
                "ChatAnswer": chatbot_response,
                "UserInput": user_speech,
        ...
        params = event.get("Details", {}).get("Parameters", {}) or {}
        filler_category = params.get("filler_category")
        filler_index = params.get("filler_index")
        
        cat, idx, filler_key, filler_url = _filler_key_url(filler_category, 
                                                           int(filler_index) if filler_index else None)
        
        _log("Selected filler audio", category=cat, index=idx, key=filler_key)
        
        # 2. Set up cache entry for the full audio
        cache_key = _make_key(uuid.uuid4().hex)
        created = ddb_put_pending_if_absent(pk, COMPANY_BUCKET, cache_key, chatbot_response)
        
        if created:
            _log("Created cache entry for full audio", pk=pk[:12] + "...", cache_key=cache_key)
            ...
            try:
                ...
                _log("Background full synthesis worker started", pk=pk[:12] + "...")
            except Exception as e:
        ...
        response_attrs = {
            ...
            "AudioS3UrlCount": "1",
            "BatchCount": "1",
            "HasMore": "true",  # Indicate there will be more (the full audio)
            "NextIndexOut": "1",
            "ChatAnswer": chatbot_response,
            "UserInput": user_speech,
            "pk": pk,
...
def _synthesize_full_worker(event):
    ...
    This is called asynchronously after the filler audio is returned.
    """
    try:
        text = (event.get("text") or "").strip()
        pk = event.get("pk") or ""
        ...
        if not text or not pk or not key:
            _log("synthesize_full_worker: missing required parameters", 
                 has_text=bool(text), has_pk=bool(pk), has_key=bool(key))
            return {"ok": False}
        ...
        try:
            prompt_id, prompt_arn = _ensure_connect_prompt_for_key(bucket, key, pk[:12])
        ...
        return {"ok": True, "bucket": bucket, "key": key, "prompt_arn": prompt_arn}
...
def _check_full_ready(event):
    """
    Check if the full audio (generated by synthesize_full_worker) is ready.
    This is called after the filler audio has played.
    """
    params = event.get("Details", {}).get("Parameters", {}) or {}
    pk = params.get("pk") or event.get("pk") or ""
    ...
    _log("Checking full audio readiness", pk=pk[:12] + "...")
    ...
    if state == "ready":
        ...
        s3_url = f"https://{bucket}.s3.{S3_REGION}.amazonaws.com/{key}"
        # # Try presigned URL first, fall back to regional URL
        # try:
        #     s3_url = _s3.generate_presigned_url(
        #         "get_object",
        #         Params={"Bucket": bucket, "Key": key},
        #         ExpiresIn=300
        #     )
        #     _log("Using presigned URL for full audio", key=key)
        # except Exception as e:
        #     _log("Presigned URL failed, using regional URL", error=str(e))
        #     s3_url = f"https://{bucket}.s3.{S3_REGION}.amazonaws.com/{key}"
        ...
        return {
            ...
            "setAttributes": {
                ...
                "AudioS3UrlCount": "1",
                "BatchCount": "1",
                "HasMore": "false",  # This is the final complete audio
                "NextIndexOut": "1",
                "prompt_arn": prompt_arn,
                "final_text": final_text,
                "job_bucket": bucket,

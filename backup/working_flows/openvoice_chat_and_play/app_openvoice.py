...
import os, io, uuid, base64, re, time, tempfile, json
...
def _tts_warm_inference():
    ...
    try:
        ...
        ref = os.getenv("OV_WARMUP_REF") or os.getenv("OV_DEFAULT_REF") or ""
...
# where to store cached target embeddings (optional)
OV_TGT_SE_DIR = os.getenv("OV_TGT_SE_DIR", "checkpoints_v2/target_speakers/ses")
os.makedirs(OV_TGT_SE_DIR, exist_ok=True)

# default reference (choose ONE: a WAV or an already-saved .pth)
OV_DEFAULT_REF   = os.getenv("OV_DEFAULT_REF", speaker_wav) or os.getenv("OPENVOICE_DEFAULT_REF", speaker_wav)  # path to .wav
...
WS_BASE   = os.getenv("WS_BASE",   "a30904a4985f.ngrok-free.app")
...
OV_SPEED         = float(os.getenv("OV_SPEED", "1.35"))  # default speaking rate
...
def _synthesize_chunk_to_key(chunk: str, target_sr: int, job_id: str, idx: int, speaker_wav: Optional[str] = speaker_wav) -> str:
    eng = get_engine()
    with torch.inference_mode():
        audio_f32, model_sr = eng.synthesize(text=chunk, speaker_wav=speaker_wav, sr=target_sr)
...
#     float_mono, _ = _to_float_mono(audio_f32, model_sr, target_sr)
    ...
    float_mono = spectral_denoise(float_mono,
        sr=24000,
        noise_ms=200,      # shorter profile (less risk of “learning” speech)
        reduce_db=9.0,     # lighter overall attenuation
        floor_db=-26.0,    # leave a slightly higher noise floor (avoids hollow artifacts)
        time_smooth=2,     # less temporal smearing of plosives/nasals
        freq_smooth=2      # keep consonant edges
    )
...
def synthesize_stream(req: SynthesizeIn, authorization: Optional[str] = Header(None)):
    ...
    for sent in sentences:
        audio_f32, model_sr = eng.synthesize(text=sent, speaker_wav=req.speaker_wav, sr=req.sample_rate)
        float_mono, _ = _to_float_mono(audio_f32, model_sr, target_sr)

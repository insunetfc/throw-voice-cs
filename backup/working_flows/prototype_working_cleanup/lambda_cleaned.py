...
DISABLE_DDB = os.getenv("DISABLE_DDB", "1") == "1"
        ...
        if DISABLE_DDB:
            ...
            try:
                ...
                if head_response.get("ContentLength", 0) > 0:
                    ...
                    s3_url = f"https://{TTS_BUCKET}.s3.{CONNECT_REGION}.amazonaws.com/{cache_key}"
                    
                    _log("S3 cache hit for full synthesis - returning immediately", cache_key=cache_key)
                    
                    attrs = {
                        "ready": "true",
                        "AudioS3Url0": s3_url(TTS_BUCKET, f"{raw_cat}/{raw_idx:02d}.wav"),
                        "AudioS3UrlCount": "1",
                        "BatchCount": "1",

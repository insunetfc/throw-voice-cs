    ...
    <div class="card">
      ...
      <div id="fish-panel">
        ...
        <div class="slider-container">
          ...
          <input id="temp" type="range" min="0.1" max="1.0" step="0.05" value="0.90" />
        </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const ttsTemp        = document.getElementById("temp");
  const ttsTempLabel   = document.getElementById("temp-value");
  ...
  function applyTonePreset(tone) {
    ...
    const t = tone || "";
    let newTemp = "0.7";  // default / [None]

    // “happy” styles → hotter / more random
    if (t === "(happy)" || t === "(excited)" || t === "(friendly)") {
      newTemp = "0.9";
    }
    // “sad / fearful / angry” → cooler / more flat
    else if (t === "(sad)" || t === "(fearful)" || t === "(angry)") {
      newTemp = "0.4";
    }
    // "" (None) → mid
    else if (t === "") {
    ...
    ttsTemp.value = newTemp;
    if (ttsTempLabel) {
      ttsTempLabel.textContent = newTemp;
    }
  ...
  toneBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // store the currently selected tone string, e.g. "(happy)"
  ...
  sendBtn?.addEventListener("click", async () => {
    ...
    const baseText = (ttsText.value || "").trim();
    if (!baseText) {
      alert("텍스트를 입력해 주세요.");
      return;
    }

    const textWithTone = currentTone
    ? `${currentTone} ${baseText}`
    : baseText;

...
def call_brain(user_text: str, owner: str, voice_id: str = None, use_gpt_voice: bool = True) -> dict:
    """Call GPT+ElevenLabs brain."""
    # Use GPT Voice endpoint by default
    if use_gpt_voice:
        url = f"{BRIDGE_BASE}/voice/brain/gpt-voice-eleven"
    else:
        url = f"{BRIDGE_BASE}/voice/brain/gpt-eleven"
    
    payload = {"text": user_text, "owner": owner}
    if voice_id:
        payload["voice_id"] = voice_id
...
def main():
    ...
    parser.add_argument("--use-text-gpt", action="store_true", help="Use text GPT instead of GPT Voice")
    ...
    try:
        ...
        # Brain (GPT + Eleven)
            ...
            result = call_brain(text, args.owner, voice_id=args.voice_id, use_gpt_voice=not args.use_text_gpt)
            
            print(f"\n{'='*70}")
            print(f"🤖 GPT Reply: {result['reply_text']}")
            if result.get('tts_text') and result['tts_text'] != result['reply_text']:

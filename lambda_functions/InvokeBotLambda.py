...
CHAT_URL = os.getenv("CHAT_URL", "https://honest-trivially-buffalo.ngrok-free.app/chatbot/respond")
# CHAT_URL = os.getenv("CHAT_URL", "https://99c0ad659065.ngrok-free.app/respond")
CHAT_TOKEN = os.getenv("CHAT_TOKEN", "")
TTS_URL = os.getenv("TTS_URL", "https://honest-trivially-buffalo.ngrok-free.app/tts/synthesize")
...
def decide_post_action_from_intent(model_intent: str) -> str:
    """
    Map model-level intent -> what the call flow should do next.
    You can tune this mapping anytime without touching the model.
    """
    if not model_intent:
        return "continue"

    # End the call when the customer clearly rejects
    if model_intent in {"rejection"}:
        return "end"

    # Positive / closing-like intents – here I assume we also end the call nicely.
    # If you later want SMS or something, change to "sms_end" etc.
    if model_intent in {"positive"}:
        return "end"

    # Fallback: unclear / need clarification -> keep going
    if model_intent in {"fallback"}:
        return "continue"

    # For informational stuff, keep conversation alive
    if model_intent in {"about_company", "fee_question", "more_questions", "greeting", "other"}:
        return "continue"

    # Default: be safe and continue
    return "continue"
...
def _unified_voice_response_handler_ultra_fast(event):
    """
    Ultra-fast handler that:
      - uses KR overrides (namecard / thanks)
      - calls retriever chatbot for text + audio URL
      - uses model's predicted_intent to decide post_action
      - NEVER generates presigned URLs
    """
    ...
    # 1) KR hard overrides (namecard / polite closing) – keep these
    ...
    # 2) Call retriever chatbot (no cache)
    try:
        ...
        # Audio fields from NIPA side (after you wire CSV URLs)
        audio_http_url = (
            chat_resp.get("chosen_audio_http_url")
            or chat_resp.get("audio_http_url")
        )
        chosen_audio_s3_uri = (
            chat_resp.get("chosen_audio_s3_uri")
            or chat_resp.get("audio_s3_uri")
        )

        # Model intent – this is the key for branching
        model_intent = chat_resp.get("predicted_intent") or "other"

    except Exception as e:
        _log("Chatbot call failed", error=str(e))
    ...
    # 3) Audio decision (no presigned URLs)
    try:
        # Prefer direct HTTP URL from bank
        if audio_http_url:
            audio_url = audio_http_url

        # Else, raw S3 URI
        elif chosen_audio_s3_uri:
            audio_url = chosen_audio_s3_uri

        # Else, TTS fallback (still raw S3 URI)
        else:
            ...
            audio_url = tts_s3_uri

    except Exception as e:
        _log("Audio selection failed", error=str(e))
        neutral_url = get_neutral_audio_url("general", index=random.randint(1, 3))
        return {
            "ready": "true",
            "AudioS3Url0": neutral_url,
    ...
    # 4) Decide call flow from model intent
    post_action = decide_post_action_from_intent(model_intent)

    # 5) Final return to Connect
    return {
        "ready": "true",
        "AudioS3Url0": audio_url,
        "AudioS3UrlCount": "1",
